...
import { IntentRouteExternalSchema } from "@/workflow/external/intent/intent.schema";
...
export const intentRouteExternal: FastifyPluginAsyncTypebox = async (app) => {
  ...
  app.route({
    method: "POST",
    url: "/intent",
    schema: IntentRouteExternalSchema,
    handler: async (req, reply) => {
      try {
        ...
        if (isRecordNotFoundError(err)) {
          throw new NotFoundError(`No resource was found for ${JSON.stringify(req.body)}`);
        }
