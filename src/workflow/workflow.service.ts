...
export const ResubmissionReasons = [
  ResubmissionReason.BLURRY_IMAGE,
  ResubmissionReason.CUT_IMAGE,
  ResubmissionReason.UNSUPPORTED_DOCUMENT,
  ResubmissionReason.DAMAGED_DOCUMENT,
  ResubmissionReason.EXPIRED_DOCUMENT,
  ResubmissionReason.COPY_OF_A_COPY,
  ResubmissionReason.FACE_IS_UNCLEAR,
  ResubmissionReason.FACE_IS_NOT_MATCHING
];

export type TResubmissionReason = typeof ResubmissionReasons[number];
...
export class WorkflowService {
  ...
  private formatWorkflow(workflow: TWorkflowWithRelations) {
    ...
    const service = createWorkflow({
      ...
      definitionType: workflow.workflowDefinition.definitionType as "statechart-json" | "bpmn-json",
      workflowContext: {
  ...
  private _resolveOrderByParams(
    ...
    orderDirection: typeof SortOrder[keyof typeof SortOrder] | undefined
  ): object {
  ...
  async handleRuntimeFinalState(
    ...
    logger.info(`Entity state updated to ${ApprovalState.PROCESSING}`, {
  ...
  async createOrUpdateWorkflowRuntime({
    ...
    logger.info(existingWorkflowRuntimeData ? "Workflow updated" : "Workflow created", {
    ...
    return await Promise.all(
      document.pages.map(async documentPage => {
        const ballerineFileId =
          ...
          (await this.__copyFileToDestinationAndCreateFile(document, entityId, documentPage));
  ...
  private __validateWorkflowDefinitionContext(
    ...
    // @ts-expect-error TODO: fix type
    const validate = ajv.compile(workflowDefinition?.contextSchema?.schema);
  ...
  async event({
    ...
    logger.info("Workflow event received", { id, type });
