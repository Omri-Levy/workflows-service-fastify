...
import { ResubmitFlowRouteSchema } from "@/collection-flow/resubmit-flow/resubmit-flow.schema";
import { env } from "@/env";
...
export const resubmitFlowRoute: FastifyPluginAsyncTypebox = async (app) => {
  ...
  app.route({
    method: "POST",
    url: "resubmit/:flowId",
    schema: ResubmitFlowRouteSchema,
    handler: async (req, reply) => {
      const projectIds = getProjectIds(req);
      const resubmittedFlow = await collectionFlowService.resubmitFlow(req.params.flowId, projectIds);

      return reply.send(resubmittedFlow)
    }
  });
};