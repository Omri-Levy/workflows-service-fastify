...
import {
  GetFlowConfigurationRouteSchema
} from "@/collection-flow/get-flow-configuration/get-flow-configuration.schema";
import { env } from "@/env";
...
export const getFlowConfigurationRoute: FastifyPluginAsyncTypebox = async (app) => {
  ...
  app.route({
    ...
    schema: GetFlowConfigurationRouteSchema,
    handler: async (req, reply) => {
      const flowConfiguration = await collectionFlowService.getFlowConfiguration(req.query.flowType);

      return reply.send(flowConfiguration)
    }
  });
};