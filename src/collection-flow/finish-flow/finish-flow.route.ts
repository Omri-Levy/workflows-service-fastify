...
import { FinishFlowRouteSchema } from "@/collection-flow/finish-flow/finish-flow.schema";
...
export const finishFlowRoute: FastifyPluginAsyncTypebox = async (app) => {
  ...
  app.route({
    method: "POST",
    url: "/finish/:flowId",
    schema: FinishFlowRouteSchema,
    handler: async (req, reply) => {
      const projectIds = getProjectIds(req);
      const finishedFlow = await collectionFlowService.finishFlow(req.params.flowId, projectIds);

      return reply.send(finishedFlow)
    }
  });
};